...
vec3 vec3::random()
{
    return vec3(Defs::random_double(), Defs::random_double(), Defs::random_double());
}

vec3 vec3::random(double min, double max)
{
    return vec3(Defs::random_double(min, max), Defs::random_double(min, max), Defs::random_double(min, max));
}
...
vec3 vec3::random_in_unit_disk()
{
    while (true)
    {
        auto p = vec3(Defs::random_double(-1, 1), Defs::random_double(-1, 1), 0);
        if (p.length_squared() >= 1) continue;
        return p;
    }
}
